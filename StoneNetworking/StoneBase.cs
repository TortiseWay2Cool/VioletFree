        ...
        public static void NetworkedTag()
        {
            foreach (VRRig rig in GorillaParent.instance.vrrigs)
            {
                if (TagsEnabled)
                {
                    ...
                    string label = "";
                    if (Cha.Contains(PhotonNetwork.LocalPlayer.UserId))
                        label = "Stone Owner";
                    else if (Tortise.Contains(PhotonNetwork.LocalPlayer.UserId))
                        label = "Stone Owner";
                    else if (ADuserid.Contains(PhotonNetwork.LocalPlayer.UserId))
                        label = "Stone Admin";
                    else if (IIAdminuserid.Contains(PhotonNetwork.LocalPlayer.UserId))
                        label = "Console Admin";
                        ...
                        label = "Evicted User";
                    else if (p.CustomProperties.TryGetValue("ControlFreeUser", out object CF))
                        label = "Control Free User";
                    else if (p.CustomProperties.TryGetValue("controlpaid", out object CP))
